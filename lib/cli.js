...
var tasks = [],
    ...
    onResource = function(err, data) {
        if(hasData(err, data)) {
            ...
            data.data = core.removeDuplicates(data.data);
        }
    ...
    tasks.push(function(cb) {
        ...
        if(utils.isUrl(resource)) {
            if(utils.isSitemap(resource)) {
                core.checkSitemap(resource, function() {
                    cb();
                }, settings, onResource);
            } else {
                core.checkUrl(resource, function(err, data) {
                    onResource(err, data);
                    cb();
                }, settings);
            }
                    ...
                    utils
                        ...
                        .forEach(function(file) {
                            subTasks.push(function(subcb) {
                                core.checkFile(file, function(err, data) {
                                    onResource(err, data);
                                    subcb();
                                }, settings);
                            });
